...
select
  ...
  coalesce(sum(ii.quantity * p.cost), 0) as total_cost_value,
  coalesce(sum(ii.quantity * p.price), 0) as total_sale_value,
  count(distinct case when coalesce(sum(ii.quantity), 0) <= p.min_stock then p.id end) as low_stock_count
from products p
left join inventory_items ii on ii.product_id = p.id
